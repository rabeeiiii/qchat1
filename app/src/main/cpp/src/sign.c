...
// Alice starts by :
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) { // creating pointer to public key and and pointer to secret key
 // making variables :
  ...
  polyvecl mat[K]; // Matrix A with size k ( 4X4 )
 ...
 // k and l are constraints to distinguish dimensions of matrix  (generate random seeds to be used in public and private key creation)
  seedbuf[SEEDBYTES+0] = K; 
  seedbuf[SEEDBYTES+1] = L; 
  // expand the randomness in seedbuf into a longer sequence of random bytes.
  shake256(seedbuf, 2*SEEDBYTES + CRHBYTES, seedbuf, SEEDBYTES+2); // a hashing function of sha-3 family , is a variable length hash function of (p => SEEDBYTES , p' => CRHBYTES , k => another SEEDBYTES)
...
// why do we expand ?
  ...
  // ( A x S1 ) + S2     this will be odne on 2 parts , firts the multiplication , then the addition
  //part 1 - ( A x S1 )
  //part 2 -    result + S2  
  s1hat = s1; // preserving the original s1
  polyvecl_ntt(&s1hat); //apply ntt to s1hat 
  polyvec_matrix_pointwise_montgomery(&t1, mat, &s1hat); //part 1- multiply matrix a by s1 , store in t1 , part of the public key 
  polyveck_reduce(&t1); // ??????????????????????????????
  polyveck_invntt_tomont(&t1); // ????????????????????????

  /* Add error vector s2 */
  polyveck_add(&t1, &t1, &s2); //part 2- add error vector s2 to t1 , hide exact structure of secret vector ( learning with errors )

  /* Extract t1 and write public key */
  //compute t0 and t1 using power2round
  polyveck_caddq(&t1);  // applying central reduction to keep values within the expected boundaries
  polyveck_power2round(&t1, &t0, &t1); // split t1 into t1 and t0 ( t1 -> higher order bits , t0 -> lower order bits )
 
  pack_pk(pk, rho, &t1); // combinine the public key => 256 bit used to expand matrix A using rho
