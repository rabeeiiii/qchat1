...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    // Step 1: Generate randomness for rho, rhoprime, and key
    ...
    // Step 2: Expand matrix
    polyvec_matrix_expand(mat, rho);

    // Step 3: Sample short vectors s1 and s2
    polyvecl_uniform_eta(&s1, rhoprime, 0);
    polyveck_uniform_eta(&s2, rhoprime, L);

    // Step 4: Compute t1 = A * s1 + s2
    s1hat = s1;
    polyvecl_ntt(&s1hat);
    ...
    // Step 5: Extract t1 and store public key
    polyveck_caddq(&t1);
    polyveck_power2round(&t1, &t0, &t1);
    pack_pk(pk, rho, &t1);

    // Step 6: Compute H(rho, t1) and store secret key
